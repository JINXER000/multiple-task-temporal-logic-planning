...
# Robots
MODEL_DIRECTORY = '/home/joseph/yzchen_ws/task_planning/AI-plan/pybullet-planning/models'
...
WSG_50_URDF = join_paths(MODEL_DIRECTORY,
                         'drake/wsg_50_description/urdf/wsg_50_mesh_visual.urdf')  # wsg_50 | wsg_50_mesh_visual | wsg_50_mesh_collision
...
def getLabeled_configuration(body):
...
# # defined by joseph
# def get_center_extent(lower, upper):
#     center = [(lower[i]+ upper[i])/2.0 for i in range(len(lower))]
#     extent = [(upper[i]- lower[i]) for i in range(len(lower))]
...
def plan_nonholonomic_motion(body, joints, end_conf, obstacles=[], attachments=[],
    ...
    path = solve(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn,
                 algorithm=algorithm,
                 **kwargs)  # weights=weights, # TODO: deliberately excluding for PRM unless circular
    # return path
...
def smooth_path(robot, joints, path, obstacles=[], attachments=[],
    ...
    collision_fn = get_collision_fn(robot, joints, obstacles, attachments, self_collisions, disabled_collisions,
                                    custom_limits=custom_limits, use_aabb=use_aabb, cache=cache,
                                    max_distance=max_distance)
    max_velocities, max_accelerations = get_dynamical_limits(robot, joints, max_velocities, max_accelerations,
                                                             duration_to_max)
